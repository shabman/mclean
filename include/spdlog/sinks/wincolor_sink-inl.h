...
wincolor_sink<ConsoleMutex>::print_range_(const memory_buf_t& formatted, size_t start, size_t end)
{
    if (end > start)
    {
        ...
        auto ignored = ::WriteConsoleW(
            static_cast<HANDLE>(out_handle_), wformatted.data(), size, nullptr, nullptr);
#else
...
wincolor_sink<ConsoleMutex>::write_to_file_(const memory_buf_t& formatted)
{
    ...
    auto  ignored       = ::WriteFile(
        static_cast<HANDLE>(out_handle_), formatted.data(), size, &bytes_written, nullptr);
    (void) (ignored);
}
