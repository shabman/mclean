...
auto
get_path_string(const std::filesystem::path& p, const std::basic_string<PathChar>& native)
{
...
template <typename Char, typename PathChar>
void
write_escaped_path(basic_memory_buffer<Char>&         quoted,
...
auto
write_escaped_alternative(OutputIt out, const T& v) -> OutputIt
{
...
class is_variant_formattable_
{
    ...
    static std::conjunction<is_formattable<std::variant_alternative_t<Is, T>, C>...>
...
auto
write_demangled_name(OutputIt out, const std::type_info& ti) -> OutputIt
{
...
auto
ptr(const std::unique_ptr<T, Deleter>& p) -> const void*
{
    return p.get();
}
template <typename T>
auto
ptr(const std::shared_ptr<T>& p) -> const void*
{
    return p.get();
}
...
public:
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char*
    {
        ...
        return parse_format_specs(
            ctx.begin(), ctx.end(), specs_, ctx, detail::type_constant<T, Char>::value);
    }
