...
FMT_CONSTEXPR auto
lossless_integral_conversion(const From from, int& ec) -> To
{
    ec      = 0;
...
template <typename To, typename From, FMT_ENABLE_IF(!std::is_same<From, To>::value)>
FMT_CONSTEXPR auto
safe_float_conversion(const From from, int& ec) -> To
{
    ec      = 0;
    using T = std::numeric_limits<To>;
...
auto
safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from, int& ec) -> To
{
...
inline auto
get_classic_locale() -> const std::locale&
{
...
template <typename CodeUnit>
void
write_codecvt(codecvt_result<CodeUnit>& out, string_view in, const std::locale& loc)
{
...
auto
write_encoded_tm_str(OutputIt out, string_view in, const std::locale& loc) -> OutputIt
{
...
auto
write_tm_str(OutputIt out, string_view sv, const std::locale& loc) -> OutputIt
{
...
inline void
do_write(buffer<Char>& buf, const std::tm& time, const std::locale& loc, char format, char modifier)
{
...
template <typename Char, typename OutputIt, FMT_ENABLE_IF(!std::is_same<Char, char>::value)>
auto
write(OutputIt out, const std::tm& time, const std::locale& loc, char format, char modifier = 0)
    -> OutputIt
{
...
auto
duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
{
#if !FMT_SAFE_DURATION_CAST
...
template <typename Duration>
auto
to_time_t(sys_time<Duration> time_point) -> std::time_t
{
...
localtime(std::time_t time) -> std::tm
{
    struct dispatcher
    {
        std::time_t time_;
...
template <typename Duration>
FMT_DEPRECATED auto
localtime(std::chrono::local_time<Duration> time) -> std::tm
{
...
template <typename Duration>
inline auto
gmtime(sys_time<Duration> time_point) -> std::tm
{
...
template <typename OutputIt>
auto
write_padding(OutputIt out, pad_type pad, int width) -> OutputIt
{
...
template <typename OutputIt>
auto
write_padding(OutputIt out, pad_type pad) -> OutputIt
{
    if (pad != pad_type::none)
...
FMT_CONSTEXPR auto
parse_chrono_format(const Char* begin, const Char* end, Handler&& handler) -> const Char*
{
    if (begin == end || *begin == '}')
        return begin;
    if (*begin != '%')
...
inline auto
tm_wday_full_name(int wday) -> const char*
{
...
inline auto
tm_wday_short_name(int wday) -> const char*
{
...
inline auto
tm_mon_full_name(int mon) -> const char*
{
...
inline auto
tm_mon_short_name(int mon) -> const char*
{
    static constexpr const char* short_name_list[] = {
        "Jan",
        "Feb",
...
inline auto
to_nonnegative_int(T value, Int upper) -> Int
{
...
constexpr auto
pow10(std::uint32_t n) -> long long
{
...
void
write_fractional_seconds(OutputIt& out, Duration d, int precision = -1)
{
...
template <typename Duration>
void
write_floating_seconds(memory_buffer& buf, Duration duration, int num_fractional_digits = -1)
{
...
inline auto
get_milliseconds(std::chrono::duration<Rep, Period> d) -> std::chrono::duration<Rep, std::milli>
{
...
auto
format_duration_value(OutputIt out, Rep val, int) -> OutputIt
{
...
auto
format_duration_value(OutputIt out, Rep val, int precision) -> OutputIt
{
...
auto
copy_unit(string_view unit, OutputIt out, Char) -> OutputIt
{
...
template <typename OutputIt>
auto
copy_unit(string_view unit, OutputIt out, wchar_t) -> OutputIt
{
...
auto
format_duration_unit(OutputIt out) -> OutputIt
{
